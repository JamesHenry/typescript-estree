...
module.exports = function calculateProjectParserOptions(code, options) {
    ...
    const compilerHost = ts.createCompilerHost(parsedCommandLine.options, /* setParentNodes */ true);
    const oldGetSourceFile = compilerHost.getSourceFile;
    compilerHost.getSourceFile = (filename, languageVersion, onError, shouldCreateNewFile) => 
        path.normalize(filename) === path.normalize(options.filePath) 
            ? ts.createSourceFile(filename, code, languageVersion, true) 
            : oldGetSourceFile(filename, languageVersion, onError, shouldCreateNewFile);
